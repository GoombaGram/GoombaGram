...
import (
	"encoding/binary"
	"github.com/GoombaGram/GoombaGram/GoombaGram/internal/crypto/aes"
)
...
func (abr *Abridged) Send(data []byte) error {
	...
	if length >= 127 {
		...
		binary.LittleEndian.PutUint32(byteLength, length)
		byteLength = byteLength[:len(byteLength)-1]
		...
		data = append(append([]byte{0x7F}, byteLength...), data...)
	} else {
